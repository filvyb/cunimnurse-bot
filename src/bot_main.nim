...
proc get_role_id_by_name(guild_id, role_name: string): Future[string] {.async.} =
  var role = await query.get_role_id_name(guild_id, role_name)
  if role.isSome:
    return role.get()
  if role.isNone:
...
proc figure_channel_users(c: GuildChannel): Future[seq[string]] {.async.} =
  ...
  for x, y in over_perms:
    if y.kind == 1 and permViewChannel in y.allow:
      res.add(y.id)
    if y.kind == 0:
      let rol_users = await query.get_all_role_users(c.guild_id, y.id)
      if rol_users.isSome:
...
proc give_user_sis_role(user: DbUser): Future[bool] {.async.} =
  ...
  for g in guild_ids:
    ...
    if not await query.exists_role_relation(g, user.id, (await get_role_id_by_name(g, conf.discord.verified_role))):
      continue
    try:
...
proc give_user_sis_role(user_id: string): Future[bool] {.async.} =
  var user = await query.get_user(user_id)
  if user.isNone:
    return false
...
proc give_all_users_sis_role(): Future[(int, int)] {.async.} =
  var users = await query.get_verified_users()
  if users.isNone:
    return (0,0)
  ...
  for us in users.get():
    ...
    if user.study_type == "":
      if not (await parse_sis_for_user(user, true)):
        ...
        user = (await query.get_user(user.id)).get()
...
proc set_reaction2thread(m: Message, emoji_name: string, thread_id: string, message_id: string) {.async.} =
  ...
  if room_id in conf.discord.thread_react_channels:
    if await insert_thread_reaction(m.guild_id.get(), emoji_name, room_id, thread_id, message_id):
...
proc sync_channels(guild_id: string) {.async.} =
  ...
  if db_channels.isSome:
    ...
    for c in db_chan_to_del:
      if await query.delete_channel(guild_id, c):
        info(fmt"Deleted channel {c} in guild {guild_id} from DB")

    for c in disc_chan_to_add:
      if await query.insert_channel(guild_id, c):
        info(fmt"Added channel {c} in guild {guild_id} to DB")

  for ch in discord_channels:
    if ch.kind == ctGuildText or ch.kind == ctGuildForum:
      let disc_ch_users_set = toHashSet(await figure_channel_users(ch))
      let db_ch_users_seq = await query.get_all_channel_users(guild_id, ch.id)

      if db_ch_users_seq.isNone:
        for u in disc_ch_users_set:
          if await query.insert_channel_membership(guild_id, u, ch.id):
      ...
      if db_ch_users_seq.isSome:
        ...
        for u in db_user_to_del:
          if await query.delete_channel_membership(guild_id, u, ch.id):
...
proc sync_roles(guild_id: string) {.async.} =
  ...
  var db_roles = await query.get_all_roles(guild_id)
  # populates empty db
  if db_roles.isNone:
    info("Role DB empty")
    for r in discord_roles:
      ...
      if await query.insert_role(guild_id, role_id, role_name, power):
  ...
  if db_roles.isSome:
    ...
    for r in db_roles_to_delete:
      if await query.delete_role(guild_id, r):
    ...
    for r in discord_roles:
      ...
      if not await query.get_role_bool(guild_id, role_id):
        if await query.insert_role(guild_id, role_id, role_name, power):
          info(fmt"Added role {role_id} {role_name} in guild {guild_id} to DB")
  ...
  for x in guild_members:
    ...
    let tmpq = await query.get_all_user_roles(guild_id, user_id)
    ...
    for r in to_delete:
      if await query.delete_role_relation(guild_id, user_id, r):
        info(fmt"Deleted role {r} from user {user_id} {user_name} in guild {guild_id} from DB")

    for r in to_add:
      if await query.insert_role_relation(guild_id, user_id, r):
...
cmd.addSlash("verify") do (login: string):
  ...
  if i.channel_id.get() == conf.discord.verify_channel:
    if conf.email.use_mail:
      var res = await query.insert_user(user_id, login, 0)
      ...
      var code = await get_verification_code(user_id)
      try:
...
cmd.addSlash("resetverify") do ():
  ...
  if i.channel_id.get() == conf.discord.verify_channel:
    var user_stat = await query.get_user_verification_status(user_id)
...
cmd.addSlash("search create") do (message: string):
  ...
  let user_id = i.member.get().user.id
  let last_id = await query.get_last_channel_search_id(guild_id, channel_id)
...
cmd.addSlash("search list") do ():
  ...
  let searches = await query.get_channel_searches(guild_id, channel_id)
  if searches.isNone:
...
cmd.addSlash("search del") do (id: int):
  ...
  if (await query.get_search_id_user(guild_id, channel_id, id)) != user_id and (await query.get_user_power_level(guild_id, user_id)) <= 1:
    await i.reply("Nemůžeš odstranit cizí příspěvky")
    return

  if await query.delete_search(guild_id, channel_id, id):
...
cmd.addChat("help") do ():
  if msg.guild_id.isNone:
    return
  if (await query.get_user_power_level(msg.guild_id.get(), msg.author.id)) <= 2:
    return
  let text = """
            Pomoc pro adminy:
...
cmd.addChat("forceverify") do (user: Option[User]):
  ...
  if (await query.get_user_power_level(guild_id, msg.author.id)) <= 2:
    return
  if user.isSome():
    var user_id = user.get().id
    var ver_stat = await query.get_user_verification_status(user_id)
    ...
    if ver_stat < 2:
      ...
      randomize()
      discard query.delete_user(user_id)
      var q = await query.insert_user(user_id, fmt"forced_{$rand(1..100000)}", 2)
      if q == false:
...
cmd.addChat("change_role_power") do (id: string, power: int):
  if msg.guild_id.isNone:
    return
  let guild_id = msg.guild_id.get()
  if (await query.get_user_power_level(guild_id, msg.author.id)) <= 3:
    return
  var res = await query.update_role_power(guild_id, id, power)
...
cmd.addChat("jail") do (user: Option[User]):
  ...
  if user.isSome():
    ...
    let q = await query.update_verified_status(user_id, 4)
    if q == false:
      discard await msg.reply("Příkaz selhal")
      return
...
cmd.addChat("unjail") do (user: Option[User]):
  ...
  if user.isSome():
    let user_id = user.get().id
    let q = await query.update_verified_status(user_id, 2)
    if q == false:
      discard await msg.reply("Příkaz selhal")
      return
    for g in guild_ids:
...
cmd.addChat("make-teacher") do (user: Option[User]):
  ...
  if user.isSome():
    let user_id = user.get().id
    let q = await query.update_user_position(user_id, 3)
    if q == false:
      discard await msg.reply("Příkaz selhal")
      return
    for g in guild_ids:
...
cmd.addChat("add-role-reaction") do (emoji_name: string, role_id: string, message_id: string):
  ...
  if room_id in conf.discord.reaction_channels:
    if await query.insert_role_reaction(guild_id, emoji_name, room_id, role_id, message_id):
      await discord.api.addMessageReaction(room_id, message_id, emoji_name)
...
cmd.addChat("add-channel-reaction") do (emoji_name: string, target_id: string, message_id: string):
  ...
  let room_id = msg.channel_id
  if room_id in conf.discord.reaction_channels:
    if await query.insert_chan_reaction(guild_id, emoji_name, room_id, target_id, message_id):
      await discord.api.addMessageReaction(room_id, message_id, emoji_name)
      discard await msg.reply("Povoleno")
    else:
      discard await msg.reply("Nastala chyba")
...
cmd.addChat("remove-emoji-reaction") do (emoji_name: string, message_id: string):
  ...
  let room_id = msg.channel_id
  if room_id in conf.discord.reaction_channels:
    if await query.delete_role_emoji_reaction(guild_id, emoji_name, room_id, message_id):
      await discord.api.deleteMessageReaction(room_id, message_id, emoji_name)
      discard await msg.reply(fmt"{emoji_name} smazáno.")
      return
    if await query.delete_chan_emoji_reaction(guild_id, emoji_name, room_id, message_id):
      await discord.api.deleteMessageReaction(room_id, message_id, emoji_name)
      discard await msg.reply(fmt"{emoji_name} smazáno.")
      return
  else:
...
cmd.addChat("msg-to-room-role-react") do (message_id: string, category_id: string):
  ...
  if room_id in conf.discord.reaction_channels:
    ...
    for l in lines:
      ...
      if await query.insert_role_reaction(guild_id, emoji, room_id, therole.id, message_id):
...
cmd.addChat("whois") do (user_id: string):
  ...
  var user_db = await query.get_user(user_id)
  if user_db.isSome:
...
cmd.addChat("reboot") do ():
  if msg.guild_id.isNone:
    return
  if (await query.get_user_power_level(msg.guild_id.get(), msg.author.id)) <= 3:
    return
...
cmd.addChat("create-role-everywhere") do (role_name: string, role_position: int, role_color: string):
  ...
  for g in guild_ids:
    ...
    try:
      ...
      if role_position < 0:
        var roles = (await query.get_all_roles(g)).get()
...
proc guildRoleCreate(s: Shard, g: Guild, r: Role) {.event(discord).} =
  ...
  if await query.insert_role(g.id, role_id, role_name, 1):
...
proc guildRoleDelete(s: Shard, g: Guild, r: Role) {.event(discord).} =
  ...
  if await query.delete_role(g.id, role_id):
...
proc guildRoleUpdate(s: Shard, g: Guild, r: Role, o: Option[Role]) {.event(discord).} =
  ...
  if role_name != role_name_old:
    if await query.update_role_name(g.id, role_id, role_name):
...
proc guildMemberAdd(s: Shard, g: Guild, m: Member) {.event(discord).} =
  ...
  if (await query.get_user_verification_status(user_id)) == 2:
...
proc guildMemberRemove(s: Shard, g: Guild, m: Member) {.event(discord).} =
  ...
  if (await query.delete_all_user_role_relation(g.id, user_id)):
...
proc messageReactionAdd(s: Shard, m: Message, u: User, e: Emoji, exists: bool) {.event(discord).} =
  ...
  if room_id in conf.discord.reaction_channels:
    # Assign role
    var role_to_give = await query.get_reaction_role(guild_id, emoji_name, room_id, msg_id)
    if role_to_give != "":
      try:
    ...
    var channel_to_give = await query.get_reaction_chan(guild_id, emoji_name, room_id, msg_id)
    if channel_to_give != "":
  ...
  if room_id in conf.discord.thread_react_channels:
    var thread_to_give = await query.get_reaction_thread(guild_id, emoji_name, room_id, msg_id)
    if thread_to_give != "":
...
proc messageReactionRemove(s: Shard, m: Message, u: User, r: Reaction, exists: bool) {.event(discord).} =
  ...
  if m.guild_id.isSome:
    guild_id = m.guild_id.get()
  var q = await query.get_all_user_roles(guild_id, user_id)
  if q.isSome:
    member_roles = q.get()
  ...
  if room_id in conf.discord.reaction_channels:
    ...
    var role_to_del = await query.get_reaction_role(guild_id, emoji_name, room_id, msg_id)
    ...
    var channel_to_del = await query.get_reaction_chan(guild_id, emoji_name, room_id, msg_id)
  ...
  if room_id in conf.discord.thread_react_channels:
    var thread_to_del = await query.get_reaction_thread(guild_id, emoji_name, room_id, msg_id)
    if thread_to_del != "":
...
proc threadDelete(s: Shard, g: Guild, c: GuildChannel, exists: bool) {.event(discord).} =
  ...
  if c.kind == ctGuildPrivateThread:
    ...
    let thread_q = await query.get_react_msg_by_thread(g.id, c.parent_id.get(), thread_id)
    if thread_q.isNone:
      return
    try:
      ...
      error(e.msg)
    if await query.delete_reaction_thread(g.id, thread_id):
...
proc guildMemberUpdate(s: Shard; g: Guild; m: Member; o: Option[Member]) {.event(discord).} =
  ...
  var q = await query.get_all_user_roles(g.id, user_id)
  ...
  for r in to_delete:
    if await query.delete_role_relation(g.id, user_id, r):
      info(fmt"Deleted role {r} from user {user_id} {user_name} in guild {g.id} from DB")

  for r in to_add:
    if await query.insert_role_relation(g.id, user_id, r):
  ...
  if g.isSome:
    if c.isSome:
      if c.get().kind == ctGuildText or c.get().kind == ctGuildForum:
        if await query.insert_channel(g.get().id, c.get().id, c.get().name):
...
proc channelDelete(s: Shard, g: Option[Guild], c: Option[GuildChannel], d: Option[DMChannel]) {.event(discord).} =
  if g.isSome:
    if c.isSome:
      if c.get().kind == ctGuildText or c.get().kind == ctGuildForum:
        if await query.delete_channel(g.get().id, c.get().id):
...
proc channelUpdate(s: Shard, g: Guild, c: GuildChannel, o: Option[GuildChannel]) {.event(discord).} =
  if c.kind == ctGuildText or c.kind == ctGuildForum:
    ...
    let disc_users_set = toHashSet(await figure_channel_users(c))
    let db_users = await query.get_all_channel_users(g.id, c.id)

    if db_users.isNone:
      for u in disc_users_set:
        if await query.insert_channel_membership(g.id, u, channel_id):
    ...
    if db_users.isSome:
      ...
      for u in db_user_to_del:
        if await query.delete_channel_membership(g.id, u, channel_id):
...
proc guildBanAdd(s: Shard, g: Guild, u: User) {.event(discord).} =
  discard await query.update_verified_status(u.id, 3)
...
proc guildBanRemove(s: Shard, g: Guild, u: User) {.event(discord).} =
  discard await query.delete_user(u.id)
  for gid in guild_ids:
    if gid != g.id:
      try:
...
proc messageCreate(s: Shard, msg: Message) {.event(discord).} =
  ...
  if ch_type[1].isSome:
    ...
    if (await check_msg_for_verification_code(content, author_id)) == true:
      discard await query.update_verified_status(author_id, 2)
      for g in guild_ids:
...
proc messageDelete(s: Shard, m: Message, exists: bool) {.event(discord).} =
  ...
  if ch_type[0].isSome:
    ...
    if room_id in conf.discord.thread_react_channels:
      var threads = await query.get_threads_by_message(guild_id, room_id, msg_id)
      if threads.isNone:
        return
